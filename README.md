...
If you generated an instrumental track (with `[instrumental]` in the lyrics), the Vocals stem is skipped because it would only contain silence and separation noise. You get four clean stems instead of five.
...
Supports WAV, MP3, OGG, FLAC, AIFF, M4A, and WMA files.
...
Effects: Envelope, Gate, High Pass, EQ, Compressor, Saturation, Bitcrusher, Stereo Width, Delay, and Reverb.
...
### Chord Detection
Chords are detected automatically when you extract stems, or you can right-click any audio file in the Assets panel and choose **Detect Chords**. A colored bar above the waveform shows the detected chord progression.

### MIDI Export (Early Access)
Click **Export MIDI** to convert the current clip to a MIDI file. Also available via right-click in the Assets panel. Accuracy is highest on isolated melody and bass stems. MIDI files appear in the Assets panel under the MIDI category.
...
You have 6 channels. Drag audio files from the Assets panel into any channel, or use Extract Stems to automatically separate a song into vocals, drums, bass, and more.
...
### The Timeline
The bottom area shows your tracks on a timeline. Each clip appears as a waveform block. Click on a waveform to select that channel. Use the zoom buttons to zoom in and out. Right-click a clip for options like copy, paste, loop, and per-clip effects. All edits are non-destructive: your original files are never modified.
...
- **Waveform** — Animated frequency visualizer. Real-time spectrum analysis driven by the actual audio playing.
- **Lines** — Timed lyrics as subtitles at the bottom of the screen. Only the current line is visible, like movie subtitles.
- **Sing Along** — Karaoke mode with word-by-word highlighting. The current line shows with each word changing color as it is sung. The next line previews below.
...
### Licenses
The Licenses page lists every piece of open-source software that rAIdio.bot uses, along with each one's license type. This is here for transparency.
...
*For the full in-app help including advanced controls, press F1 from any tab.*
